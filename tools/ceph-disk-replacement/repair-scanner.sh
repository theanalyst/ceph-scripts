...
do
  drain=`timeout 10 ssh -oStrictHostKeyChecking=no $i ls /tmp/log.drain.* 2> /dev/null`;
  replt=`timeout 10 ssh -oStrictHostKeyChecking=no $i ls /tmp/log.prepare.* 2> /dev/null`;
