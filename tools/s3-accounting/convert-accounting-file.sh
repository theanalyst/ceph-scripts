...
do
  ...
  echo "$line" | jq --arg DATE `date "+%d-%m-%Y"` --arg CHARGEGRP "$charge_group" -c '. | { name: .display_name, DiskUsage: .usage, DiskQuota: .quota, Date: $DATE, Owner: .owner, charge_group: $CHARGEGRP }'
done
