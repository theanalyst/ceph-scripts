...
for uid in users:
    ...
    if info['user_quota']['max_size_kb'] > 1:
        ...
        affiliation = "CERN N/A"
        print '%s (%s): %s quota, %s used, %d buckets, %d objects, %s' % (info['display_name'], uid, sizeof_fmt(info['user_quota']['max_size']), sizeof_fmt(stats['total_bytes']), len(buckets), stats['total_entries'], affiliation)
