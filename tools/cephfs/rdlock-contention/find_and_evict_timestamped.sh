...
now=$(date)
end=$(date --date 'now + 1 minutes' +%T) # Seconds may be better? 'now + 40 seconds'

loglines=$(tail -n 20 -f $logfile | sed "/$end/ q" | grep "failed to rdlock") # fetch a few lines prior with -n so we dont miss any in the processing window
inodes=$(echo $loglines | awk '{print $18}' | cut -d '/' -f 1 | sort | uniq | grep "^#0x" | tr -d '#')
